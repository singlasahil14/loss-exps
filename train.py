...
class _ClassifierModel(Network):
  ...
  def _localization_network(self, x):
    with tf.variable_scope('localization'):
      ...
      x = self._dense(self, 32, name='dense1')(x)
      x = self._tanh(x)
...
class LossMinimizer:
  def __init__(self, model_config):
    assert type(train_tuple)==type(test_tuple)==tuple
    assert len(train_tuple)==len(test_tuple)==2
    sess_config = tf.ConfigProto()
    sess_config.gpu_options.allow_growth = True
    self._sess = tf.Session(config=sess_config)

    (x_train, y_train), (x_test, y_test) = load_data(options.dataset)
    self._x_train, self._y_train = x_train, y_train
    self._x_test, self._y_test = x_test, y_test

    num_classes = y_train[1]
    self._images = tf.placeholder(tf.float32, [None] + x_train.shape()[1:])
    self._labels = tf.placeholder(tf.float32, [None, num_classes])

    model = _ClassifierModel()
    model.forward(images, num_classes, stn=model_config.use_stn, nonlin=model_config.nonlin)
  ...
  def run_optimization(self, num_epochs, checkpoint_iters=10, batch_size=128, result_path=None):
    ...
    for epoch_i in range(num_epochs):
      ...
      while i < len(batch_size):
        ...
        if self._iters % checkpoint_iters == 0:
          print('Iteration: ' + str(self._iters) + ' Loss: ' + str(loss))
      ...
      print('Test Accuracy: %g, Test Cross Entropy: %g' % test_accuracy, test_cross_entropy)

def add_arguments(parser):
  parser.add_argument('--dataset', choices=['cluttered-mnist', 'fashion-mnist', 'cifar10', 'cifar100'], default='cifar10', type=str, 
...
def main():
  ...
  loss_minimizer.run_optimization(num_epochs=options.num_epochs, checkpoint_iters=options.checkpoint_iters, batch_size=options.batch_size)
